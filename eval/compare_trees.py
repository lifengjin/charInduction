...
def main(sysargs):
    ...
    sent_recall = []
    with torch.no_grad():
        for k, (tree1, tree2) in enumerate(zip(open(args.gold, "r"), open(args.pred))):
            if tree1.count('(') <= 1 or tree2.count('(') <= 1:
                continue
            tree1 = tree1.strip()
            ...
            sent_f1.append(f1)
            sent_recall.append(reca)
    tp, fp, fn = corpus_f1
    prec = tp / (tp + fp)
    recall = tp / (tp + fn)
    corpus_f1 = 2 * prec * recall / (prec + recall) if prec + recall > 0 else 0.
    logging.info('Corpus Recall: %.2f, Sentence Recall: %.2f' % (recall * 100, np.mean(np.array(sent_recall)) * 100))
    logging.info('Corpus F1: %.2f, Sentence F1: %.2f' % (corpus_f1 * 100, np.mean(np.array(sent_f1)) * 100))
    return recall, corpus_f1, np.mean(np.array(sent_recall)), np.mean(np.array(sent_f1))
